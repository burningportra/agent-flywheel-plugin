...
- **`PANE001` lint rule (`mcp-server/src/lint/rules/pane001.ts`).** Parses AGENTS.md for the canonical "NTM pane priority" block (cc:cod:gem 1:1:1 + substitution ladder + version-pin marker like "as of v3.17.0"), scans every `skills/start/_*.md` + `skills/start/SKILL.md` for pane-spawn occurrences (`--cc=`, `--cod=`, `--gmi=`, `--pi=`, narrative pane-count phrases), and flags any disagreement. Severity defaults to `warn` for one release cycle to match the `RESERVE001` rollout pattern; `lint:skill` job emits GHA annotations but does not hard-block CI until the v3.19 promotion. Registered in `mcp-server/scripts/lint-skill.ts`; tests in `mcp-server/src/__tests__/lint-rule-pane001.test.ts`. (claude-orchestrator-34ok)
- **`flywheel_remediate codex_config_compat` handler (`mcp-server/src/tools/remediate.ts`).** Auto-comments the offending `model = "gpt-5*" / "gpt-5-codex"` line in `~/.codex/config.toml` with a millisecond-precision timestamped backup at `~/.codex/config.toml.bak.<ISO-timestamp>`. Follows the existing `buildPlan / execute / verifyProbe` pattern; default mode is `dry_run`; pass `mode: "execute"` + `autoConfirm: true` to actually mutate. Closes the `null` slot that previously left `codex_config_compat` flagged-but-unfixable. (claude-orchestrator-3s58)
- **`gemini_model_compat` doctor check.** New entry in `DOCTOR_CHECK_NAMES` (bringing the sweep to **22 checks**, up from 21). Parses `gemini --version` reachable model against an allowlist defined in `mcp-server/src/model-detection.ts` (currently `gemini-3.1-flash-preview`-only per upstream CLI constraints). Yellow when configured model is outside the allowlist; the new pre-flight model-config gate uses this signal to downgrade `--gmi` lanes before spawn. (claude-orchestrator-37n6)
- **Pre-flight model-config gate (`skills/start/_inflight_prompt.md`, `_implement.md`, `_planning.md`, `_reality_check.md`, `_deslop.md`).** Every pane-spawning pre-flight now reads `codex_config_compat` + `gemini_model_compat` and downgrades the `--cod` or `--gmi` share to `--cc` per the substitution ladder when either check is non-green, with a one-line warning to the coordinator log. Fixes the 2026-05-15 `gpt-5.5-xhigh` 400 crash in which a fresh swarm spawn crashed on the first `cod` pane before any work could begin. Also updates `mcp-server/src/outcome-grading.ts` `defaultGraderDriver` to preempt the same misconfig before invoking `codex exec`. (claude-orchestrator-2wcd)
- **Yellow doctor checks now surface above the Step 0d menu** (was red-only). `skills/start/SKILL.md` Step 0c now lists every check with severity ∈ {yellow, red} above the menu instead of hiding yellow signal behind a "run flywheel-doctor" pointer. Glyph differentiation: red = ✗, yellow = ⚠. (claude-orchestrator-6bxt)

### Changed

- **NTM pane priority — canonical mixed `cc:cod:gem` 1:1:1.** Single canonical rule across AGENTS.md (v3.12.0 cod-first rule retired with a "Superseded in v3.17.0" marker — kept the marker phrasing since the rule actually landed in v3.17.0's documentation) + every downstream skill body: `skills/start/_inflight_prompt.md` (verbatim prompt + operator decoder table + Windows fallback), `skills/start/_reality_check.md` (Full-pipeline option description + §4 verbatim prompt + §4b spawn command), `skills/start/SKILL.md` (Step 0d previous-session-exists + open-beads-exist printed blocks AND their AskUserQuestion descriptions — they used to disagree with each other), `skills/start/_implement.md` + `_planning.md` + `_deslop.md` (spawn rationale). Substitution ladder when a provider is unavailable: `gmi → cod → pi → cc`. (claude-orchestrator-ao9u + 2gno + 2dcn + 1ogv + 3ihi)
- **README sweep-count + skill-count drift reconciled.** `21-check sweep` → `22-check sweep` at three sites (lines 58, 214, 327 originally; 214 was folded into the troubleshooting table). Skill / phase-sub-file counts re-verified against filesystem (`40 SKILL.md`, `10 phase sub-files including _tutorial_bead.md`). (claude-orchestrator-1ch2 + q8xi + 1zzz + 1gew)
- **Two stale proposal docs archived.** `docs/skill-refine-flywheel-proposed.md` and `docs/skill-refine-start-proposed.md` moved to `docs/archive/` with disposition tables noting which items shipped via which v3.18 bead and which (if any) are still outstanding. (claude-orchestrator-2i2t + 3ibw + 2ekf)
- **Stale v3.15.0 checkpoint cleared.** `.pi-flywheel/checkpoint.json` had been claiming `phase: implementing` with 24 `activeBeadIds` from the v3.16.0 noob-onboarding mega-release that already shipped at HEAD `f5fa76f`. Cleared as part of the reality-check wrap-up. (claude-orchestrator-1m3q)
- **`mcp-server/dist/` rebuilt and committed.** v3.16.0 release commit `13090ab` (named "release: v3.16.0 — noob-onboarding turnkey installer + tutorial-bead") had left `dist_drift` red post-merge — 24 src files were newer than `dist/server.js`. Rebuild + commit closes the gate. (claude-orchestrator-2plp)

### Compatibility

- No breaking API changes. `PANE001` is `warn`-only for v3.18; pre-existing skill bodies that already adopt the canonical block stay green.
- `gemini_model_compat` adds one entry to `DOCTOR_CHECK_NAMES`; checkpoints written by v3.17.0 continue to load unchanged (no required state fields added).
- `flywheel_remediate codex_config_compat` defaults to `dry_run`; no surprise mutations of `~/.codex/config.toml`.
- The mixed `cc:cod:gem` 1:1:1 rule preserves backward-compat with `cc`-only spawns (the substitution ladder collapses to all-cc when both Codex and Gemini are unavailable). Existing `--cc=N` invocations continue to work.

### Audit verdict

`flywheel_compliance_audit` (mode: single-bead, threshold: 700) on the five code beads landed **PASS 5/5** with mean score **909/1000** (min 855, max 970). All in the "Substantially complete" band; no false-closed work. Six follow-ups filed under label `reality-check-2026-05-15-followup` (5×P3 + 1×P2) covering: `PANE001` warn→error promotion, override-marker tightening, AGENTS.md `FW_GRADER_MODEL safe-default cascade` subsection, backup-collision unit test, audit-parser/REPORT.md mismatch, CHANGELOG pane-priority entry (this entry closes that one).
