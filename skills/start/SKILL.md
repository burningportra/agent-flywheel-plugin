...
| **Other** | Surface a follow-up `AskUserQuestion` with the state-appropriate sub-options (per UNIVERSAL RULE 1, never end the turn here). For previous-session-exists state: `Work on beads / New goal / Deslop pass / Research repo`. For open-beads-exist state: `Deslop pass / New goal / Research repo / Audit`. For fresh-start state: `Deslop pass / Research repo / Setup`. Reality check is now top-level on every menu state (previous-session, open-beads, fresh-start), so it is intentionally NOT duplicated in the sub-menu. Deslop pass moves to the sub-menu in the previous-session and fresh-start states (it remains top-level on open-beads). Quick fix is reachable via `/agent-flywheel:flywheel-fix` directly and intentionally not in the sub-menu. Then route the chosen sub-option through the matching row below. |
