...
const CONFIGS = {
    br_binary: {
        ...
        installCommand: 'curl -fsSL "https://raw.githubusercontent.com/Dicklesworthstone/beads_rust/main/install.sh" | bash',
    },
    bv_binary: {
        binary: 'bv',
        description: 'Install bv (beads viewer) via the official install script.',
        installCommand: 'curl -fsSL "https://raw.githubusercontent.com/Dicklesworthstone/beads_viewer/main/install.sh" | bash',
    },
    ntm_binary: {
        binary: 'ntm',
        description: 'Install ntm (named tmux manager) via the official install script.',
        installCommand: 'curl -fsSL "https://raw.githubusercontent.com/Dicklesworthstone/ntm/main/install.sh" | bash',
    },
    cm_binary: {
        binary: 'cm',
        description: 'Install cm (CASS Memory) via the official install script.',
        installCommand: 'curl -fsSL "https://raw.githubusercontent.com/Dicklesworthstone/cass_memory_system/main/install.sh" | bash -s -- --easy-mode --verify',
    },
};
function buildHandler(checkName) {
    ...
    return {
        ...
        async verifyProbe(ctx) {
            ...
            try {
                const versionRes = await ctx.exec('bash', ['-lc', `command -v ${cfg.binary} >/dev/null 2>&1 && ${cfg.binary} --version`], { cwd: ctx.cwd, timeout: VERIFY_TIMEOUT_MS, signal: ctx.signal });
                if (versionRes.code === 0)
                    return true;
                // Fallback: some CLIs (ntm) don't accept --version. Treat any
                // command-resolves-to-something as green.
                const helpRes = await ctx.exec('bash', ['-lc', `command -v ${cfg.binary} >/dev/null 2>&1 && ${cfg.binary} --help`], { cwd: ctx.cwd, timeout: VERIFY_TIMEOUT_MS, signal: ctx.signal });
...
export const brBinaryHandler = buildHandler('br_binary');
export const bvBinaryHandler = buildHandler('bv_binary');
export const ntmBinaryHandler = buildHandler('ntm_binary');
export const cmBinaryHandler = buildHandler('cm_binary');
//# sourceMappingURL=cli_binary.js.map