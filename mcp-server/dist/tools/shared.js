 ...
 */
export function resolveExecutionMode(coordinationMode, hasAgentMail) {
    if (coordinationMode === 'single-branch')
        return 'single-branch';
    if (coordinationMode === 'worktree')
        return 'worktree';
 ...
 */
export function computeConvergenceScore(polishChanges, outputSizes) {
    if (polishChanges.length < 3)
        return 0;
 ...
 */
export function formatRepoProfile(profile) {
    const lines = [];
 ...
 */
export function computeBeadQualityScore(beads) {
    if (beads.length === 0)
        return { score: 0, label: '⛔ no beads', weakBeads: [] };
    let totalScore = 0;
    const weakBeads = [];
    for (const bead of beads) {
        ...
        if (titleScore < 20)
            issues.push('title not a verb phrase');
        ...
        if (WHAT_WHY_HOW.test(bead.description))
            descScore = Math.min(35, descScore + 5);
        beadScore += descScore;
        if (descLen < 50)
        ...
        beadScore += fileScore;
        if (fileScore < 20)
            issues.push('no file paths specified');
        ...
        if (issues.length > 0)
            weakBeads.push(`${bead.id} (${issues.join(', ')})`);
    }
 ...
 */
export function formatBeadQualityScore(q) {
 ...
 */
export function implementerInstructions(bead, profile, prevResults, cassMemory, episodic) {
