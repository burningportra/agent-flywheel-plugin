 ...
 */
export async function runDoctorChecks(cwd, signal, options = {}) {
    ...
    const checks = settled.map((r, idx) => {
    ...
    const budgetExceeded = combined.aborted && !externallyAborted && elapsedMs >= totalBudgetMs;
 ...
 * a single number.
 */
export function countCriticalFails(checks) {
 ...
 */
function severityFromCounts(checks, criticalFails) {
    if (criticalFails > 0)
        return 'red';
    if (checks.some((c) => c.severity === 'yellow'))
        return 'yellow';
    return 'green';
}
/**
 ...
 */
export function computeOverallSeverity(checks) {
    if (checks.some((c) => c.severity === 'red'))
        return 'red';
    if (checks.some((c) => c.severity === 'yellow'))
        return 'yellow';
    return 'green';
...
async function checkAgentMailLiveness(exec, cwd, signal, timeout, now) {
    const start = now();
    if (signal.aborted)
        return abortedCheck('agent_mail_liveness');
    try {
        const res = await exec('curl', ['-s', '--max-time', '2', 'http://127.0.0.1:8765/health/liveness'], { timeout, cwd, signal });
        if (res.code !== 0) {
            return {
...
async function checkBinary(checkName, binary, exec, cwd, signal, timeout, now, opts) {
    const start = now();
    if (signal.aborted)
        return abortedCheck(checkName);
    try {
...
async function checkNodeVersion(exec, cwd, signal, timeout, now) {
    const start = now();
    if (signal.aborted)
        return abortedCheck('node_version');
    try {
...
async function checkGitStatus(exec, cwd, signal, timeout, now) {
    const start = now();
    if (signal.aborted)
        return abortedCheck('git_status');
    try {
...
async function checkDistDrift(cwd, signal, now) {
    const start = now();
    if (signal.aborted)
...
async function checkCheckpointValidity(cwd, signal, now) {
    const start = now();
    if (signal.aborted)
 ...
 */
async function checkSwarmModelCli(checkName, provider, capsPromise, signal, now) {
    const start = now();
    if (signal.aborted)
        return abortedCheck(checkName);
    try {
 ...
 */
async function checkSwarmModelRatio(capsPromise, signal, now) {
    const start = now();
    if (signal.aborted)
        return abortedCheck('swarm_model_ratio');
    try {
        const caps = await capsPromise;
        const description = describeCapabilities(caps);
        const availableCount = ['claude', 'codex', 'gemini'].reduce((acc, p) => acc + (caps[p].available ? 1 : 0), 0);
        if (availableCount === 3) {
            return {
...
async function checkRescuesLast30d(exec, cwd, signal, timeout, now) {
    const start = now();
    if (signal.aborted)
        return abortedCheck('rescues_last_30d');
    try {
 ...
 */
export function countRescueEntriesWithin30Days(raw, nowMs) {
    if (!raw.trim())
        return 0;
    let parsed;
    try {
...
async function checkVersionTriple(cwd, signal, now, opts) {
    const start = now();
    if (signal.aborted)
        return abortedCheck('npm_marketplace_version_drift');
    const localPath = join(cwd, 'mcp-server', 'package.json');
    const marketplacePath = opts.marketplaceManifestPath === null
        ? null
        : opts.marketplaceManifestPath ?? join(cwd, '.claude-plugin', 'plugin.json');
    const installedPath = opts.installedManifestPath === null
        ? null
