 ...
 */
import { existsSync, mkdirSync, renameSync, statSync, unlinkSync, copyFileSync, } from "node:fs";
 ...
 */
export function isFlywheelManagedPath(absPath, cwd) {
    ...
    if (resolved.startsWith(tmpRoot + sep)) {
        ...
        if (firstSeg.startsWith(FLYWHEEL_TMP_PREFIX))
            return true;
    }
    return false;
}
 ...
 */
function denied(absPath, cwd) {
    return {
        ok: false,
        resolvedPath: resolve(absPath),
        reason: "target_outside_allowlist",
        detail: `Refusing destructive op on '${absPath}': not inside any flywheel-managed ` +
 ...
 */
export function guardedUnlink(absPath, cwd) {
 ...
 */
export function backupThenReplace(absPath, cwd) {
    const resolved = resolve(absPath);
    if (!existsSync(resolved))
        return "";
 ...
 */
export function guardedRename(srcPath, destPath, cwd) {
    if (!isFlywheelManagedPath(srcPath, cwd))
        return denied(srcPath, cwd);
    if (!isFlywheelManagedPath(destPath, cwd))
        return denied(destPath, cwd);
 ...
 */
export function guardedRemoveDir(absPath, cwd) {
    if (!isFlywheelManagedPath(absPath, cwd))
        return denied(absPath, cwd);
    const resolved = resolve(absPath);
    if (!existsSync(resolved)) {
