 ...
 */
export function findNonStandardIds(beads) {
    return beads
        .map(b => b.id)
        .filter(id => !isValidBeadId(id));
}
function tokenizePlanAudit(text) {
...
function scorePlanAuditSection(sectionText, bead) {
    ...
    for (const token of sectionTokens) {
        if (beadText.includes(token))
            hits++;
    }
    return hits / sectionTokens.length;
}
function summarizePlanAuditSection(body) {
    return body
        .split("\n")
        .map((line) => line.trim())
...
export function auditPlanToBeads(plan, beads) {
...
let _bvAvailable = null;
/**
 * Detects whether the `bv` CLI is available. Result is cached.
 */
export async function detectBv(exec) {
    if (_bvAvailable !== null)
        return _bvAvailable;
 ...
 */
export async function bvNext(exec, cwd) {
    if (!(await detectBv(exec)))
        return null;
 ...
 */
export async function bvPlan(exec, cwd) {
    if (!(await detectBv(exec)))
        return null;
 ...
 */
export async function readyBeads(exec, cwd) {
    const result = await brExecJson(exec, ["ready", "--json"], { timeout: 10000, cwd });
    if (!result.ok)
        return [];
    const data = result.value;
 ...
 */
export async function getBeadById(exec, cwd, id) {
 ...
 */
export async function beadDeps(exec, cwd, id) {
 ...
 */
export function extractArtifacts(bead) {
 ...
 */
export async function updateBeadStatus(exec, cwd, beadId, status) {
 ...
 * Syncs beads to disk.
 */
export async function syncBeads(exec, cwd) {
 ...
 */
export async function remediateOrphans(exec, cwd, orphanIds) {
    const closed = [];
    const failed = [];
 ...
 */
export async function validateBeads(exec, cwd) {
    let cycles = false;
    let orphaned = [];
    const warnings = [];
    const shallowBeads = [];
    ...
    if (insights) {
        ...
        const cycleOutput = (cycleResult.ok
            ...
            : cycleResult.error.stdout).toLowerCase();
        if (cycleOutput) {
            const confirmsNoCycles = cycleOutput.includes("no dependency cycles detected") ||
                cycleOutput.includes("all dependency checks passed");
            const indicatesCycles = /detected\s+cycle|cycle\s+detected|dependency\s+cycles\s+detected/.test(cycleOutput);
 ...
 */
export async function qualityCheckBeads(exec, cwd) {
    const failures = [];
    ...
    if (ready.length >= 2) {
        const artifactMap = new Map(); // file -> bead IDs
 ...
 */
export function getBeadsSummary(beads) {
    if (beads.length === 0)
        return "no beads tracked";
    let closed = 0;
    let inProgress = 0;
    let open = 0;
    let deferred = 0;
