...
export function workflowRoadmap(currentPhase) {
...
export function formatRepoProfile(profile, scanResult) {
    const sections = [
    ...
    if (scanResult?.codebaseAnalysis && (scanResult.codebaseAnalysis.summary || scanResult.codebaseAnalysis.recommendations.length > 0 || scanResult.codebaseAnalysis.structuralInsights.length > 0)) {
        ...
        if (scanResult.codebaseAnalysis.recommendations.length > 0) {
            sections.push("\n#### Recommended focus areas", ...scanResult.codebaseAnalysis.recommendations.slice(0, 5).map((item) => `- **${item.title}**${item.priority ? ` (${item.priority})` : ""}: ${item.detail}`));
        }
        if (scanResult.codebaseAnalysis.structuralInsights.length > 0) {
            sections.push("\n#### Structural insights", ...scanResult.codebaseAnalysis.structuralInsights.slice(0, 5).map((item) => `- **${item.title}**: ${item.detail}`));
        }
    }
    if (profile.todos.length > 0) {
        sections.push("\n### Notable TODOs", ...profile.todos.slice(0, 10).map((t) => `- \`${t.file}:${t.line}\` [${t.type}] ${t.text}`));
    }
    if (profile.recentCommits.length > 0) {
        sections.push("\n### Recent Commits", ...profile.recentCommits.slice(0, 10).map((c) => `- \`${c.hash}\` ${c.message} (${c.author})`));
    }
    if (profile.readme) {
        const truncated = profile.readme.length > 2000
...
export function flywheelSystemPrompt(hasSophia, coordBackend) {
...
// ─── Discovery Prompt ────────────────────────────────────────
export function discoveryInstructions(profile, scanResult) {
...
3. **Score each candidate** against these 5 axes (1-5 scale):
   - **Useful** (2x weight) - does it solve a real, frequent pain?
   - **Pragmatic** (2x weight) - is it realistic to build in hours/days?
   - **Accretive** (1.5x weight) - does it clearly add value beyond what exists?
   - **Robust** (1x weight) - will it handle edge cases and work reliably?
   - **Ergonomic** (1x weight) - does it reduce friction or cognitive load?
...
export function beadCreationPrompt(goal, repoContext, constraints) {
...
export function formatPlanToBeadAuditWarnings(audit) {
    if (audit.uncoveredSections.length === 0 && audit.weakMappings.length === 0) {
        return "";
    }
    const lines = ["** Plan-to-bead audit warnings**"];
    if (audit.uncoveredSections.length > 0) {
        lines.push("Uncovered plan sections:", ...audit.uncoveredSections.slice(0, 5).map((section) => `- **${section.heading}**${section.summary ? ` - ${section.summary}` : ""}`));
    }
    if (audit.weakMappings.length > 0) {
        lines.push("Weak section-to-bead mapping:", ...audit.weakMappings.slice(0, 5).map((section) => {
...
export function planToBeadsPrompt(planPath, goal, profile) {
...
export function beadRefinementPrompt(roundNumber, priorChanges) {
...
export function freshContextRefinementPrompt(cwd, goal, roundNumber, simulationReport) {
 ...
 */
export function simulationRefinementPrompt(report, beadIds) {
 ...
 */
export function computeConvergenceScore(changes, outputSizes, descriptionSnapshots) {
    if (changes.length < 3)
        return 0;
...
export function synthesisInstructions(plans) {
...
export function realityCheckInstructions(goal, beads, results) {
...
function normalizePromptSection(content, heading) {
...
export function implementerInstructions(bead, profile, previousResults, cassMemory, episodicContext) {
...
export function reviewerInstructions(bead, implementationSummary, profile, episodicContext) {
    const criteriaLines = bead.description
        .split("\n")
...
export function adversarialReviewInstructions(bead, implementationSummary, domainExtras) {
    const criteriaLines = bead.description
        .split("\n")
        .filter((line) => line.trim().startsWith("- [ ]") || line.trim().startsWith("- [x]"))
        .map((line) => line.trim().replace(/^- \[.\] /, ""));
...
export function crossAgentReviewInstructions(goal, beads, results) {
...
export function polishInstructions(goal, artifacts) {
...
export function commitStrategyInstructions(beads, results) {
...
export function skillExtractionInstructions(goal, artifacts) {
...
export function strategicDriftCheckInstructions(goal, beads, results, completedCount, totalCount) {
...
${beads.map((b) => {
        const r = results.find((r) => r.beadId === b.id);
        return `- ${b.id}: ${r?.status === "success" ? "[done]" : r ? "[in-progress]" : "[open]"} ${b.title}`;
    }).join("\n")}
 ...
 */
export function blunderHuntInstructions(cwd, passNumber, domainExtras) {
...
export function randomExplorationInstructions(goal, changedFiles, cwd) {
...
export function deSlopifyInstructions(files) {
...
// ─── Landing the Plane ──────────────────────────────────────
export function landingChecklistInstructions(cwd) {
...
export function swarmMarchingOrders(cwd, beadId) {
...
export const CODEX_SUBAGENT_TYPE = "codex:codex-rescue";
 ...
 */
export function beadQualityScoringPrompt(beadId, title, description) {
...
export function researchInvestigatePrompt(externalUrl, projectName, cwd) {
    const repoSlug = externalUrl.replace(/\.git$/, "").split("/").slice(-2).join("-");
    const cloneDir = `/tmp/research-${repoSlug}`;
...
export function researchDeepenPrompt() {
 ...
 */
export function researchHandoffPrompt(externalName, selectedGoal, artifactName, phasesCompleted, totalPhases, hasRepoProfile) {
...
export function researchInversionPrompt(projectName, externalName) {
...
export function goalRefinementPrompt(goal, profile) {
...
export function summaryInstructions(goal, beads, results) {
...
export function competingPlanAgentPrompt(focus, goal, profile, scanResult, cassContext) {
    const repoContext = formatRepoProfile(profile, scanResult);
    const memorySection = cassContext && cassContext.trim().length > 0
...
export function planSynthesisPrompt(plans, format = "markdown") {
...
export function planDocumentPrompt(goal, profile, scanResult) {
    const repoContext = formatRepoProfile(profile, scanResult);
 ...
 */
export function planGitDiffReviewPrompt(planText) {
 ...
 */
export function planIntegrationPrompt(planText, revisions) {
...
export function planRefinementPrompt(planPath, roundNumber) {
 ...
 */
export function freshPlanRefinementPrompt(planText, planArtifactPath, roundNumber, cwd, cassContext) {
    const memorySection = cassContext && cassContext.trim().length > 0
        ? `\n## Memory from Prior Orchestrations\n${cassContext.trim()}\n`
        : "";
...
export function learningsExtractionPrompt(goal, beadIds) {
 ...
 */
export function parseBeadQualityScore(output) {
    const jsonMatch = output.match(/```json\s*([\s\S]*?)```/);
    const raw = jsonMatch ? jsonMatch[1] : output;
    try {
        const parsed = JSON.parse(raw.trim());
        if (typeof parsed.what === "number" &&
 ...
 */
export function formatBeadQualityAudit(results) {
    if (results.length === 0)
        return "No beads scored.";
    const scored = results.filter((r) => r.score !== null);
    if (scored.length === 0)
        return "WHAT/WHY/HOW scoring failed for all beads.";
    ...
    const weakestAxis = (() => {
        ...
        if (min === avgWhat)
            return "WHAT (implementation detail)";
        if (min === avgWhy)
            return "WHY (rationale/context)";
    ...
    const lines = [
        ...
        avgWhat < 3 || avgWhy < 3 || avgHow < 3
            ? `**Weakest axis: ${weakestAxis}** - focus next refinement round here.`
            : `All axes above threshold.`,
        ``,
    ];
    // List weak beads (any axis < 3)
    const weak = scored.filter((r) => r.score.what < 3 || r.score.why < 3 || r.score.how < 3);
    if (weak.length > 0) {
 ...
 */
export function auditAgentPrompt(focus, profile, files, cwd, domainExtras) {
 ...
 */
export function scanAgentPrompt(focus, files, cwd, domainExtras) {
 ...
 */
export function findingsToBeadsPrompt(findings, cwd) {
    const priority = (sev) => sev === "critical" ? "P0" : sev === "high" ? "P1" : sev === "medium" ? "P2" : "P3";
