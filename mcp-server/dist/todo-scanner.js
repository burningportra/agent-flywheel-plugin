...
async function walkFiles(root, extensions, skipDirs, signal, maxFiles = 5000) {
    const out = [];
...
export function _resetTsModuleCache() {
    tsModulePromise = null;
}
const TS_EXTS = new Set([".ts", ".tsx", ".js", ".jsx"]);
const TS_MARKER_RE = /\b(TODO|FIXME|HACK|XXX)\b\s*(?:\(.*?\))?\s*:?\s*(.*)/;
function extractTsMarker(commentText) {
...
async function scanTsFile(ts, filePath, relPath) {
    let source;
    try {
...
export const tsAstScanner = {
    name: "ts-ast",
    async scan(_exec, cwd, signal) {
 ...
 *   stay accurate).
 */
function stripPyStrings(source) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (startsTriple) {
            ...
            const isDocstring = prev < 0 || // top of file
    ...
    const stripped = lines.map((line, i) => {
        ...
        out = out.replace(/([fFrRbBuU]{0,2})(["'])((?:\\.|(?!\2).)*)\2/g, (_m, prefix, q, body) => {
...
async function scanPyFile(filePath, relPath) {
    let source;
    try {
        source = await fs.readFile(filePath, "utf8");
    }
...
export const pythonTodoScanner = {
    name: "python",
    async scan(_exec, cwd, signal) {
 ...
 */
export function mergeAndDedup(items) {
    const seen = new Set();
    const out = [];
 ...
 */
export function selectScanners(env = process.env) {
