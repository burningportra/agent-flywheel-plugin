...
export const DEFAULT_HINTS = {
    missing_prerequisite: 'A required prerequisite (CLI tool, file, or state) is missing — run `/flywheel-setup` to install dependencies, then retry.',
    invalid_input: 'The tool was called with an argument that failed schema validation — re-read the tool description and pass the documented shape.',
    ...
    unsupported_action: 'The requested action is not supported in the current context — re-read the tool description for the list of valid actions in this phase.',
    internal_error: 'An unexpected internal error occurred — capture the cause string, file an issue, and retry; this is usually transient.',
    doctor_check_failed: 'A doctor check raised an unrecoverable error — see the `cause` field for the underlying message and fix the reported issue, then re-run `/flywheel-doctor`.',
    doctor_partial_report: 'Doctor completed but some checks were skipped — review the `details.skipped` list; the remaining checks still produced a usable report.',
    hotspot_parse_failure: 'The hotspot analyzer could not parse a required input file — verify the file exists and is well-formed JSON/markdown, then retry.',
    hotspot_bead_body_unparseable: 'A bead body did not match the expected hotspot section schema — inspect the bead with `br show <id>` and reformat the body before retrying.',
    postmortem_empty_session: 'Post-mortem ran against a session with no recorded activity — confirm `.pi-flywheel/checkpoint.json` and telemetry exist, then retry.',
    postmortem_checkpoint_stale: 'The checkpoint pre-dates the analysis window — re-run the flywheel session or pass an explicit `--since` argument to widen the window.',
    template_not_found: 'The named template is not registered in the bead-template library — list available templates and verify the slug spelling before retrying.',
    template_placeholder_missing: 'A required template placeholder was not provided — see `details.missing` for the field list and pass them in the call.',
    template_expansion_failed: 'Template expansion threw mid-render — inspect `cause` for the underlying error; this is sometimes transient if the template library is reloading.',
    telemetry_store_failed: 'Could not write telemetry to disk — check filesystem permissions on `.pi-flywheel/telemetry/` and retry; transient disk contention is recoverable.',
    wave_collision_detected: 'Two beads in the same wave wrote to overlapping files — re-run the affected beads serially via `flywheel_review hit-me <bead-id>`.',
    review_mode_gate_failed: 'The review-mode autofix gate refused the change — inspect the gate findings, address each one manually, and re-run review.',
    review_headless_findings: 'Headless review surfaced findings that require human attention — read the findings list and act on each before closing the bead.',
    remediation_unavailable: 'No automated remediation exists for this check; follow the manual hint in the doctor report.',
    remediation_requires_confirm: 'Mutating remediation refused without explicit consent. Set `autoConfirm: true` after the user approves.',
    remediation_failed: 'The remediation handler exited non-zero. Inspect the captured stderr; re-run the original doctor check.',
    remediate_already_running: 'Another remediation for this check is in flight. Wait for the lock to release or call again with a different `checkName`.',
    bundle_integrity_failed: 'skills.bundle.json failed its manifestSha256 integrity check. Falling back to disk reads. Re-run `npm run build` to regenerate.',
    bundle_stale: "A bundled skill's source `.md` has changed on disk. The bundle is still served (stable-by-default); set FW_SKILL_BUNDLE=off to read live or rebuild.",
