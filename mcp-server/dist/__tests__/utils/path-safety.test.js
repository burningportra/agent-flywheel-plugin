...
import { assertSafeRelativePath, assertSafeSegment, requireSafeRelativePath, requireSafeSegment, } from "../../utils/path-safety.js";
...
describe("assertSafeRelativePath", () => {
    it("accepts a normal relative path", () => {
        ...
        if (r.ok)
            expect(r.value).toBe("docs/plans/2026-04-23-x.md");
    });
    it("rejects '../foo' (parent traversal)", () => {
        const r = assertSafeRelativePath("../foo", { root: ROOT });
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.reason).toBe("parent_traversal");
    });
    ...
    it("rejects '/etc/passwd' when relative is expected", () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.reason).toBe("absolute_when_relative_expected");
    });
    ...
    it("rejects non-string input", () => {
        const r = assertSafeRelativePath(42, { root: ROOT });
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.reason).toBe("not_string");
    });
...
describe("assertSafeSegment", () => {
    ...
    it("rejects '..' literal", () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.reason).toBe("reserved_segment");
    });
    it("rejects '.' literal", () => {
        const r = assertSafeSegment(".");
        expect(r.ok).toBe(false);
    ...
    it("rejects segment containing '/'", () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.reason).toBe("separator_in_segment");
    });
    it("rejects segment containing backslash", () => {
        const r = assertSafeSegment("foo\\bar");
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.reason).toBe("backslash");
    });
...
describe("require* helpers throw on rejection", () => {
    it("requireSafeRelativePath throws for '../foo'", () => {
        expect(() => requireSafeRelativePath("../foo", { root: ROOT })).toThrow(/parent_traversal/);
    });
    ...
    it("requireSafeRelativePath returns the normalized value on accept", () => {
        expect(requireSafeRelativePath("docs/plans/x.md", { root: ROOT })).toBe("docs/plans/x.md");
    });
});
