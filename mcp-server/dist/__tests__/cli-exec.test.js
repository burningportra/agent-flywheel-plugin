...
import { brExec, brExecJson, isTransientBrError, registerCliExecTelemetryHook, resilientExec, } from '../cli-exec.js';
beforeEach(() => {
    vi.useFakeTimers();
...
describe('brExecJson — JSON parsing wrapper', () => {
    ...
    it('validator failure surfaces as a permanent error', async () => {
        ...
        expect(result.ok).toBe(false);
        if (!result.ok)
            expect(result.error.stderr).toMatch(/invalid shape/);
    });
});
//# sourceMappingURL=cli-exec.test.js.map