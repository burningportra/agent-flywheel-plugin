...
describe('chaos/remediate-concurrent', () => {
    ...
    it('second concurrent call returns remediate_already_running', async () => {
        ...
        try {
            ...
            const winner = runRemediate({ cwd, checkName: 'dist_drift', autoConfirm: true, mode: 'execute' }, exec, ac.signal);
            // Yield once so the winner acquires the lock before we start the loser.
            await new Promise((r) => setTimeout(r, 20));
            const loser = await runRemediate({ cwd, checkName: 'dist_drift', autoConfirm: true, mode: 'execute' }, exec, ac.signal);
            expect('isError' in loser && loser.isError).toBe(true);
            const errResult = loser;
            ...
            const third = await runRemediate({ cwd, checkName: 'dist_drift', autoConfirm: true, mode: 'execute' }, makeExecFn([
                {
                    match: (cmd, args) => cmd === 'npm' && args[0] === 'run' && args[1] === 'build',
    ...
    it('global file lock blocks any second remediation while one is in flight', async () => {
        ...
        try {
            ...
            const ac = new AbortController();
            const a = runRemediate({ cwd, checkName: 'dist_drift', autoConfirm: true, mode: 'execute' }, exec, ac.signal);
            await new Promise((r) => setTimeout(r, 20));
            const b = await runRemediate({ cwd, checkName: 'agent_mail_liveness', autoConfirm: true, mode: 'execute' }, exec, ac.signal);
            expect('isError' in b && b.isError).toBe(true);
            const errResult = b;
            expect(errResult.structuredContent.data.error.code).toBe('remediate_already_running');
            ...
            const followup = await runRemediate({ cwd, checkName: 'agent_mail_liveness', autoConfirm: true, mode: 'execute' }, exec, new AbortController().signal);
...
//# sourceMappingURL=remediate-concurrent.test.js.map