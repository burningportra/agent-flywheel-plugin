...
const execFileMock = vi.fn((_cmd, _args, _opts, _callback) => {
...
describe("countCommitsSinceLastBatchReview", () => {
    ...
    it("throws when git returns non-integer output", async () => {
        ...
        await expect(countCommitsSinceLastBatchReview("/repo", "abc123")).rejects.toThrow(/non-integer count/);
    });
    it("throws when git command fails", async () => {
        execHandler = () => new Error("not a git repository");
        await expect(countCommitsSinceLastBatchReview("/repo", "abc123")).rejects.toThrow(/git rev-list failed/);
    });
});
...
describe("synthesizeBeadsFromFindings", () => {
    it("creates one bead per finding (all severities, no filter)", async () => {
        ...
        const created = await synthesizeBeadsFromFindings("/repo", state, findings, "old..new");
    ...
    it("passes auto-batch-review label and severity through to bead body", async () => {
        ...
        const state = baseState();
        await synthesizeBeadsFromFindings("/repo", state, [
            finding({
                severity: "high",
    ...
    it("leaves a valid partial-rollback record on mid-batch br create failure", async () => {
        ...
        const state = baseState();
        await expect(synthesizeBeadsFromFindings("/repo", state, findings, "old..new")).rejects.toThrow(/br create failed for finding 4\/4/);
    ...
    it("validates each finding via FindingSchema before shelling out", async () => {
        ...
        await expect(synthesizeBeadsFromFindings("/repo", state, malformed, "old..new")).rejects.toThrow();
...
describe("rollbackSynthesizedBeads", () => {
    ...
    it("falls back to br update --status closed when delete fails", async () => {
        ...
        execHandler = (_cmd, args) => {
            if (args[0] === "delete")
                return new Error("dependents block delete");
            if (args[0] === "update") {
    ...
    it("records failures when both delete AND close fall through, never throws", async () => {
        ...
        execHandler = (_cmd, args) => {
            if (args[0] === "delete")
                return new Error("delete boom");
            if (args[0] === "update")
                return new Error("update boom");
            throw new Error("unexpected br invocation");
        };
        const result = await rollbackSynthesizedBeads("/repo", ids);
        expect(result.deleted).toEqual([]);
    ...
    it("continues cleanup across mixed outcomes", async () => {
        ...
        execHandler = (_cmd, args) => {
            ...
            const id = args[1];
            seq.push(`${action}:${id}`);
            if (id === "w-1" && action === "delete")
                return "";
            if (id === "w-2" && action === "delete")
                return new Error("dep");
            if (id === "w-2" && action === "update")
                return "";
            if (id === "w-3" && action === "delete")
                return new Error("dep");
            if (id === "w-3" && action === "update")
...
//# sourceMappingURL=commit-batch.test.js.map