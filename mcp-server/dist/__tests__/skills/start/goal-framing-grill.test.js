...
describe('goal framing + grill-with-docs integration', () => {
    ...
    describe('skills/start/SKILL.md — Goal framing mode', () => {
        ...
        it('routes Set a goal through Goal framing mode', () => {
            expect(startBody).toMatch(/\|\s*\*\*Set a goal\*\*\s*\|[\s\S]*?Goal framing mode/);
        });
        it('routes Refine first and Ambiguous through Goal framing mode', () => {
            ...
            expect(startBody).toContain('always run **Goal framing mode** first (Recommended default: Grill with docs)');
        });
        it('references grill-with-docs skill path or get_skill name', () => {
            expect(startBody.includes('skills/grill-with-docs') ||
                startBody.includes('agent-flywheel:grill-with-docs')).toBe(true);
        });
...
//# sourceMappingURL=goal-framing-grill.test.js.map