...
import { WorktreePool, autoCommitWorktree, createWorktree, listWorktrees, removeWorktree, } from '../worktree.js';
/**
...
const argv = (...prefix) => (cmd, args) => cmd === 'git' && prefix.every((p, i) => args[i] === p);
describe('createWorktree', () => {
    ...
    it('returns ok:false with stderr trimmed when add fails (e.g., branch exists)', async () => {
        ...
        if (!r.ok)
            expect(r.error).toBe("fatal: A branch named 'feat' already exists.");
    });
    ...
    it('wraps thrown errors in WorktreeResult { ok:false }', async () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.error).toContain('ENOENT git');
    });
});
...
describe('listWorktrees', () => {
    ...
    it('returns ok:false with stderr on git failure', async () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.error).toBe('not a git repo');
    });
});
describe('autoCommitWorktree', () => {
    ...
    it('returns error string when git add fails', async () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.error).toMatch(/git add failed.*permission denied/);
    });
});
describe('WorktreePool', () => {
    ...
    it('acquire creates a new worktree and tracks it in state', async () => {
        ...
        expect(r.ok).toBe(true);
        if (r.ok)
            expect(r.data).toContain('step-0');
