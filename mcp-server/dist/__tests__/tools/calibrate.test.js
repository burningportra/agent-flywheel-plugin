...
import { makeExecFn } from '../chaos/_helpers.js';
...
const closedTs = (createdTs, offsetMs) => new Date(new Date(createdTs).getTime() + offsetMs).toISOString();
function makeTemplatedBead(id, template, createdTs, offsetMs) {
    return {
        id,
        title: `Bead ${id}`,
        status: 'closed',
...
function makeUntemplatedBead(id, createdTs, offsetMs) {
    return {
        id,
...
const TEMPLATED_BEADS = Array.from({ length: 10 }, (_, i) => makeTemplatedBead(`t-bead-${i}`, 'add-api-endpoint', CREATED_5D, 60 * 60_000));
// 3 untemplated beads
const UNTEMPLATED_BEADS = Array.from({ length: 3 }, (_, i) => makeUntemplatedBead(`u-bead-${i}`, CREATED_5D, 45 * 60_000));
...
function makeGitMatchStub(beadId, ts) {
    return {
        match: (cmd, args) => cmd === 'git' && args[0] === 'log' && args.some((a) => a.includes(beadId)),
...
describe('tools/calibrate — runCalibrate', () => {
    ...
    it('ratio = meanMinutes / EFFORT_TO_MINUTES[effort]', async () => {
        ...
        const beads = Array.from({ length: 5 }, (_, i) => makeTemplatedBead(`r-${i}`, 'add-api-endpoint', CREATED_5D, 90 * 60_000));
        const exec = makeExecFn([makeBrStub(beads), gitNoMatchStub]);
        const signal = new AbortController().signal;
        const report = await runCalibrate({ cwd, sinceDays: 90 }, exec, signal);
    ...
    it('proxy_started = true when git returns nothing', async () => {
        const cwd = makeCwd();
        const beads = Array.from({ length: 3 }, (_, i) => makeTemplatedBead(`proxy-${i}`, 'add-api-endpoint', CREATED_5D, 60 * 60_000));
        const exec = makeExecFn([makeBrStub(beads), gitNoMatchStub]);
        const signal = new AbortController().signal;
        const report = await runCalibrate({ cwd, sinceDays: 90 }, exec, signal);
        expect(report.rows.length).toBe(1);
...
//# sourceMappingURL=calibrate.test.js.map