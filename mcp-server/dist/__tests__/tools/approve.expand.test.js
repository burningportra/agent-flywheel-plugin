...
import { expandBeadPlanSpec, expandBeadPlanSpecs, } from "../../tools/approve.js";
const PHASE = "creating_beads";
...
describe("expandBeadPlanSpec — error branches", () => {
    it("emits template_not_found envelope on version mismatch", () => {
        const outcome = expandBeadPlanSpec({ ...FOUNDATION_SPEC, template: "foundation-with-fresh-eyes-gate@99" }, PHASE);
    ...
    it("emits template_not_found for unknown id", () => {
        const outcome = expandBeadPlanSpec({ ...FOUNDATION_SPEC, template: "does-not-exist@1" }, PHASE);
        expect(outcome.status).toBe("error");
        if (outcome.status !== "error")
            return;
        expect(outcome.code).toBe("template_not_found");
...
describe("expandBeadPlanSpecs — ordered batch expansion", () => {
    it("expands mixed specs preserving order and surfaces per-bead errors", () => {
        ...
        const outcomes = expandBeadPlanSpecs(specs, PHASE);
...
//# sourceMappingURL=approve.expand.test.js.map