...
async function loadLintModules() {
    const lintMod = (await import(`${lintBase}/index.js`));
    const registryMod = (await import(`${lintBase}/skillRegistry.js`));
    const baselineMod = (await import(`${lintBase}/baseline.js`));
    const manifestMod = (await import(`${lintBase}/manifest.js`));
    const reportersMod = (await import(`${lintBase}/reporters/index.js`));
    const auq = (await import(`${lintBase}/rules/askUserQuestion.js`));
    const slashMod = (await import(`${lintBase}/rules/slashReferences.js`));
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        switch (a) {
            ...
            case "--file": {
                const v = takeNext();
                if (v === null)
                    return { opts, error: "--file requires a path argument" };
                opts.file = v;
                break;
            }
            case "--baseline": {
                const v = takeNext();
                if (v === null)
                    return { opts, error: "--baseline requires a path argument" };
                opts.baseline = v;
                break;
            }
            case "--format": {
                const v = takeNext();
                if (v === null)
                    return { opts, error: "--format requires a value (pretty|compact|gha|json)" };
            ...
            case "--max-bytes": {
                const v = takeNext();
                if (v === null)
                    return { opts, error: "--max-bytes requires an integer argument" };
                const n = Number(v);
            ...
            case "--rule": {
                const v = takeNext();
                if (v === null)
                    return { opts, error: "--rule requires a rule id" };
                opts.rule.push(v);
                break;
            }
            default:
...
function pickRules(allRules, filter) {
    if (filter.length === 0)
        return allRules;
...
async function readSourceFile(file, maxBytes) {
...
export async function main(argv) {
    ...
    const { lintMod, registryMod, baselineMod, manifestMod, reportersMod, rules } = await loadLintModules();
    ...
    const reporterName = opts.format ?? reportersMod.selectReporter();
...
//# sourceMappingURL=lint-skill.js.map