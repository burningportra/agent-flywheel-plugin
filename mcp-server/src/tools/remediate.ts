...
import { DOCTOR_CHECK_NAMES, type DoctorCheckName } from './doctor.js';
...
export interface RemediationHandler {
  ...
  execute(ctx: HandlerCtx): Promise<{ stepsRun: number; stdout?: string; stderr?: string }>;
...
function truncate(s: string | undefined): string | undefined {
  ...
  if (buf.byteLength <= OUTPUT_CAP_BYTES) return s;
...
export async function runRemediate(
  ...
  exec: ExecFn,
  signal: AbortSignal,
): Promise<RemediationResult | ReturnType<typeof makeFlywheelErrorResult>> {
  const phase = 'doctor';
  const handler = REMEDIATION_REGISTRY[args.checkName];

  if (handler == null) {
    return makeFlywheelErrorResult('flywheel_remediate' as FlywheelToolName, phase, {
  ...
  const ctx: HandlerCtx = { cwd: args.cwd, exec, signal };
  ...
  try {
    ...
    let executed: { stepsRun: number; stdout?: string; stderr?: string };
    try {
