...
export const agentMailLivenessHandler: RemediationHandler = {
  ...
  async buildPlan(_ctx: HandlerCtx) {
    return {
      description:
        'Agent Mail must be started outside the MCP server. Manual step: ' + START_HINT,
      steps: [START_HINT],
      mutating: false,
      reversible: true,
    };
  ...
  async verifyProbe(ctx: HandlerCtx) {
    try {
      const res = await ctx.exec(
        ...
        { cwd: ctx.cwd, timeout: CURL_TIMEOUT_MS, signal: ctx.signal },
      );
      if (res.code !== 0) {
      ...
      const alive =
        trimmed.includes('"status":"alive"') || trimmed.includes('"status": "alive"');
      if (!alive) log.warn('verifyProbe: agent mail reachable but status not alive');
      return alive;
    } catch (err) {
