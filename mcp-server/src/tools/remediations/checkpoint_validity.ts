...
function checkpointAbsPath(cwd: string): string {
  return join(cwd, '.pi-flywheel', 'checkpoint.json');
}

function backupAbsPath(cwd: string, tsIso: string): string {
...
export const checkpointValidityHandler: RemediationHandler = {
  ...
  async buildPlan(ctx: HandlerCtx) {
    ...
    return {
      description:
        'Move .pi-flywheel/checkpoint.json to .pi-flywheel/checkpoint.json.bak.<timestamp> so the next session starts clean. Restore with `mv` from the .bak file.',
