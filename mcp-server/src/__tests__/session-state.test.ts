...
describe('detectSessionStage', () => {
  const createInitialState = (): FlywheelState => ({
    phase: 'idle',
    constraints: [],
    retryCount: 0,
    maxRetries: 3,
  ...
  it('detects open-beads stage when beads are present but no plan', () => {
    const state = createInitialState();
    const beads: Bead[] = [{ id: 'b1', title: 'test', description: 'desc', status: 'open', priority: 1 }];
    const stage = detectSessionStage(state, beads);
  ...
  it('detects implementing stage when beads are in-progress', () => {
    ...
    const beads: Bead[] = [{ id: 'b1', title: 'test', description: 'desc', status: 'in_progress', priority: 1 }];
    const stage = detectSessionStage(state, beads);
  ...
  it('detects complete stage when all beads are closed', () => {
    const state = createInitialState();
    state.phase = 'implementing';
    const beads: Bead[] = [{ id: 'b1', title: 'test', description: 'desc', status: 'closed', priority: 1 }];
    const stage = detectSessionStage(state, beads);
