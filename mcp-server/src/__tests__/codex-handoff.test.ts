...
function makeToolCall(name: string, outcome: RescueToolCall['outcome'] = 'error'): RescueToolCall {
  return {
...
describe('buildRescuePacket', () => {
  ...
  it('caps recent_tool_calls at MAX_RECENT_TOOL_CALLS, keeping the newest', () => {
    const calls: RescueToolCall[] = [];
    for (let i = 0; i < 25; i++) calls.push(makeToolCall(`tool-${i}`));
    const packet = buildRescuePacket({
      phase: 'impl',
      goal: 'stall sim',
...
describe('formatRescueEventForMemory', () => {
  it('emits the canonical RESCUE_EVENT_PREFIX so the doctor synthesis can count it', () => {
    const packet: RescuePacket = buildRescuePacket({
      phase: 'impl',
      goal: 'g',
...
describe('stall-detection simulation', () => {
   ...
   */
  function shouldOfferRescue(history: readonly { code: string; attempt: number }[]): boolean {
