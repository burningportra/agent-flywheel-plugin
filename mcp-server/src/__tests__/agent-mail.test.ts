...
import type { AgentMailReservation, ExecFn } from '../agent-mail.js';
 ...
 */
function makeExec(result: { code: number; stdout: string; stderr: string }): ExecFn {
 ...
 */
function makeThrowingExec(message: string): ExecFn {
  return vi.fn().mockRejectedValue(new Error(message));
}

function makeReservation(overrides: Partial<AgentMailReservation> = {}): AgentMailReservation {
...
describe('matchesReservationPath', () => {
  ...
  it('returns false when both path_pattern and path are absent', () => {
    const reservation: AgentMailReservation = {};
...
describe('unwrapRPC', () => {
  ...
  it('returns null when result is ok:false (network)', () => {
    expect(unwrapRPC({ ok: false, kind: 'network', message: 'timeout' })).toBeNull();
  });

  it('returns null when result is ok:false (parse)', () => {
    expect(unwrapRPC({ ok: false, kind: 'parse', message: 'bad json' })).toBeNull();
  });

  it('returns null when result is ok:false (rpc_error)', () => {
    expect(unwrapRPC({ ok: false, kind: 'rpc_error', message: 'not found' })).toBeNull();
  });
});
