...
vi.mock("../profiler.js", async () => {
  const actual = await vi.importActual<typeof import("../profiler.js")>(
    "../profiler.js"
  );
  return {
    ...actual,
...
type ExecBehavior = ExecResult | Error;
type SearchId = "workflow-entrypoints" | "planning-review" | "reliability-fallbacks";

interface CccExecOptions {
  help?: ExecBehavior;
  status?: ExecBehavior;
  init?: ExecBehavior;
  index?: ExecBehavior;
  searches?: Partial<Record<SearchId, ExecBehavior>>;
  defaultSearch?: ExecBehavior;
}
...
function cccSearchOutput(location: string, snippet: string): string {
  return [
    "--- Result 1 (score: 0.91) ---",
    `File: ${location}`,
    snippet,
...
async function resolveBehavior(behavior: ExecBehavior): Promise<ExecResult> {
...
function searchIdFromArgs(args: string[]): SearchId {
...
function makeCccExec(options: CccExecOptions = {}): ExecFn {
  const exec: ExecFn = vi.fn(async (cmd: string, args: string[]) => {
    ...
    if (subcommand === "search") {
      ...
      const behavior =
        options.searches?.[searchId] ??
        options.defaultSearch ??
        execResult({
          stdout: cccSearchOutput(
            `mcp-server/src/${searchId}.ts:1`,
            `${searchId} match`
          ),
        });
...
describe("scanRepo falls back to builtin when ccc throws", () => {
  ...
  it("uses the builtin profile when ccc indexing rejects", async () => {
    ...
    expect(result.sourceMetadata?.warnings).toContain(
      "Fell back from ccc to builtin scan provider."
    );
...
describe("partial ccc query failure returns partial ScanCodebaseAnalysis", () => {
  it("keeps two successful ccc searches when one query exits non-zero", async () => {
    ...
    expect(result.codebaseAnalysis.summary).toBe(
      "ccc scanned 2 codebase slices and returned 2 relevant matches."
    );
  ...
  it("retains a successful query even when it returns no matches", async () => {
    ...
    expect(result.source).toBe("ccc");
    expect(result.codebaseAnalysis.summary).toBe(
      "ccc scanned 2 codebase slices and returned 1 relevant matches."
    );
    ...
    expect(result.codebaseAnalysis.recommendations[1].detail).toContain(
      "mcp-server/src/errors.ts:12"
    );
  });
});
...
describe("all ccc queries fail and trigger builtin fallback", () => {
  ...
  it("falls back after every ccc search rejects", async () => {
    ...
    expect(result.codebaseAnalysis.summary).toBe(
      "Partial scan: fell back from ccc to builtin provider."
    );
    expect(profileRepoMock).toHaveBeenCalledTimes(2);
  });
});
...
describe("scanRepo never throws when both ccc and profiler fail", () => {
  it("returns an empty profile when ccc is unavailable and profiling rejects", async () => {
    ...
    expect(result.sourceMetadata?.warnings).toContain(
      "Profiler also failed: profiler down"
    );
