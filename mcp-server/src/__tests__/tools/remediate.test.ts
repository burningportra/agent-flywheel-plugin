...
import {
  RemediateInputSchema,
  REMEDIATION_REGISTRY,
  runRemediate,
} from '../../tools/remediate.js';
import { DOCTOR_CHECK_NAMES, type DoctorCheckName } from '../../tools/doctor.js';
...
function alwaysOkExec(): { exec: ExecFn; calls: Array<{ cmd: string; args: readonly string[] }> } {
      ...
      it(`${name}: ${isAvailable ? 'returns plan in dry_run' : 'returns remediation_unavailable'}`, async () => {
        ...
        try {
          ...
          const result = await runRemediate(
            { cwd, checkName: name as DoctorCheckName, autoConfirm: false, mode: 'dry_run' },
            exec,
            ac.signal,
          );
          ...
          if (!isAvailable) {
            ...
            const errResult = result as { structuredContent: { data: { error: { code: string } } } };
          ...
          const ok = result as {
            ...
            plan: { description: string; steps: string[]; mutating: boolean; reversible: boolean };
            durationMs: number;
          };
  ...
  describe('dry_run does not invoke exec for mutating handlers', () => {
    it('dist_drift dry_run never calls exec', async () => {
      ...
      try {
        ...
        const ac = new AbortController();
        const result = await runRemediate(
          { cwd, checkName: 'dist_drift', autoConfirm: false, mode: 'dry_run' },
          exec,
          ac.signal,
        );
        expect('check' in result).toBe(true);
  ...
  describe('mutating handler refuses execute without autoConfirm', () => {
    it('dist_drift execute mode + autoConfirm:false → remediation_requires_confirm', async () => {
      ...
      try {
        ...
        const ac = new AbortController();
        const result = await runRemediate(
          { cwd, checkName: 'dist_drift', autoConfirm: false, mode: 'execute' },
          exec,
          ac.signal,
        );
        expect('isError' in result && result.isError).toBe(true);
        const errResult = result as {
          structuredContent: { data: { error: { code: string; details?: { mutating?: boolean } } } };
        };
    ...
    it('orphaned_worktrees execute mode + autoConfirm:false → remediation_requires_confirm when orphans exist', async () => {
      ...
      try {
        ...
        const ac = new AbortController();
        const result = await runRemediate(
          { cwd, checkName: 'orphaned_worktrees', autoConfirm: false, mode: 'execute' },
          exec,
          ac.signal,
        );
        expect('isError' in result && result.isError).toBe(true);
  ...
  describe('execute mode with autoConfirm proceeds', () => {
    it('dist_drift execute + autoConfirm:true → executed:true, exec called', async () => {
      ...
      try {
        ...
        expect('check' in result).toBe(true);
        const ok = result as { executed: boolean; stepsRun: number; mode: string };
    ...
    it('mcp_connectivity execute on a populated dist is a no-op (idempotent)', async () => {
      ...
      try {
        ...
        const result = await runRemediate(
          { cwd, checkName: 'mcp_connectivity', autoConfirm: false, mode: 'execute' },
          exec,
          ac.signal,
        );
        expect('check' in result).toBe(true);
        const ok = result as { executed: boolean; stepsRun: number; verifiedGreen: boolean };
        expect(ok.executed).toBe(true);
        expect(ok.stepsRun).toBe(0);
    ...
    it('wraps a buildPlan throw into the structured error envelope', async () => {
      ...
      try {
        ...
        try {
          ...
          const ac = new AbortController();
          const result = await runRemediate(
            { cwd, checkName: 'mcp_connectivity', autoConfirm: false, mode: 'dry_run' },
            exec,
            ac.signal,
          );
          expect('isError' in result && result.isError).toBe(true);
