...
interface ComplianceAuditData {
  status: string;
  passed: Array<{ beadId: string }>;
  failed: Array<{ beadId: string }>;
}
...
function findBead(cwd: string, title: string): BrBead {
...
describeMaybe('compliance audit - integration', () => {
  it('runs the real skill against a 2-bead fixture project', async () => {
    ...
    try {
      ...
      execSyncIn(
        tmp,
        'br create "Add greeting function" --type task --description "Implement greet(name) returning Hello, name!. Add unit test." --priority 2',
      );
      execSyncIn(
        tmp,
        'br create "Add greet+farewell" --type task --description "Implement greet() AND farewell(). Both must have unit tests." --priority 2',
      );
      ...
      const exec: ToolContext['exec'] = async (bin, args, opts = {}) => {
      ...
      const data = (result.structuredContent as { data: ComplianceAuditData }).data;
      expect(data.status).toBe('ok');
      expect(data.passed).toHaveLength(1);
      expect(data.failed).toHaveLength(1);

      const passedIds = data.passed.map((passed: any) => passed.beadId);
      const failedIds = data.failed.map((failed: any) => failed.beadId);
