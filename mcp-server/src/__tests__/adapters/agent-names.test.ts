...
import {
  ...
  AGENT_NAME_NOUNS,
  AGENT_NAME_POOL_SIZE,
  allocateAgentNames,
  pickAgentName,
  randomAgentName,
} from '../../adapters/agent-names.js';

function expectAdjectiveNounName(name: string): void {
  const adjective = AGENT_NAME_ADJECTIVES.find((candidate) =>
  ...
  expect(
    AGENT_NAME_NOUNS,
    `${name} should end with a pooled noun`,
  ).toContain(name.slice(adjective!.length));
...
describe('agent-names', () => {
  it('fails fast when the requested allocation exceeds the pool capacity', () => {
    expect(() =>
      allocateAgentNames(AGENT_NAME_POOL_SIZE + 1, 'overflow'),
    ).toThrow(`requested ${AGENT_NAME_POOL_SIZE + 1} names`);
  });
