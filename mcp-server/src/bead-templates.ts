...
function defineTemplate(template: BeadTemplate): BeadTemplate {
  ...
  if (orphaned.length > 0 || unused.length > 0) {
    throw new Error(
      `Template "${template.id}@${template.version}" has mismatched placeholders.\n` +
...
function validateTemplateIntegrity(templates: BeadTemplate[]): string[] {
  ...
  const seenVersionedIds = new Set<string>();
...
export function listTemplates(): BeadTemplate[] {
 ...
 */
export function getTemplateById(templateId: string, version?: number): BeadTemplate | undefined {
 ...
 */
function inputToPlaceholders(input: TemplateExpansionInput): Record<string, string> {
...
export function expandTemplate(
  ...
  try {
    const template = BUILTIN_TEMPLATES.find(
      (candidate) => candidate.id === templateId && candidate.version === version,
    );
    if (!template) {
      return {
        success: false,
