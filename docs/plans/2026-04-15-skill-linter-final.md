...
`skills/orchestrate/SKILL.md` is **1438 lines** with **38 real `AskUserQuestion` call sites** (verified empirically via `grep -cE 'AskUserQuestion\s*\('`), ~5 hard-rule callouts, hundreds of `/slash-skill` refs, and 53+ `<placeholder>` tags. Prior incidents (bead-z9g: nested fences cascading to 100+ TS errors; UBS heuristic floods freezing merges) make the case for a linter that:

1. Enforces invariants the orchestrate skill depends on (Universal Rule 1 = every decision goes through `AskUserQuestion` with 2–4 options each carrying a `description`).
  ...
  src/
    ...
    __tests__/
      lint/
        ...
        live-orchestrate.test.ts   # canary against real SKILL.md
...
1. **Repo-local skills:** `skills/*/SKILL.md` (authoritative for orchestrate-family).
...
jobs:
  ...
  lint-skill:
    ...
    steps:
      ...
      - run: npm run build
        working-directory: mcp-server
      - run: node dist/scripts/lint-skill.js --file ../skills/orchestrate/SKILL.md --ci --baseline .lintskill-baseline.json --format gha
        working-directory: mcp-server
      - run: node dist/scripts/lint-skill.js --file ../skills/orchestrate/SKILL.md --ci --format json > lint-skill-report.json
        working-directory: mcp-server
        if: failure()
      - uses: actions/upload-artifact@v4
        if: always()
...
```json
{
  ...
  "entries": [
    { "ruleId": "PLACE001", "rulesetVersion": 1, "file": "skills/orchestrate/SKILL.md", "line": 573, "fingerprint": "sha256:abc…", "reason": "" }
  ]
}
```
...
Orthogonal to baseline. Contains `knownExternalSlashes` (pre-approved CLI built-ins like `/fast`, `/clear`, `/help`, plus `/orchestrate-*` family which lives in `skills/`) and `acceptedFindings` (narrow exceptions with required `reason:`). Adding entries needs reviewer approval (social gate).
...
**Live canary:** `live-orchestrate.test.ts` runs `lint()` against real `skills/orchestrate/SKILL.md` and asserts zero errors after baseline+allowlist. Catches regressions instantly.
...
T12: GitHub Actions CI workflow + manifest-drift guard + live canary + initial baseline | depends_on: [T11] | files: [.github/workflows/ci.yml, .nvmrc, mcp-server/.lintskill-baseline.json, mcp-server/.lintskill-manifest.json, mcp-server/.lintskill-allowlist.json, mcp-server/src/__tests__/lint/live-orchestrate.test.ts, AGENTS.md (one-para addition)] | acceptance: 4 jobs (build/test/manifest-drift/lint-skill); CI runs node dist/... (NOT tsx); manifest-drift catches missing entries; live canary asserts zero errors with baseline; AGENTS.md updated.
```
...
5. **Live canary:** `cd mcp-server && npm test` → live-orchestrate.test.ts asserts zero errors on real SKILL.md.
