...
**Scope anchor:** happy-path MVP from `docs/brainstorms/harden-the-opencode-port-of-agent-flywheel-2026-07-17.md` — a repeatable sync script plus a real Claude-ism sweep over the 23 managed skills and 25 commands. Diverged non-flywheel skills, watchers, and self-heal remain future work.
...
**MCP tool-prefix contract (`flywheel_flywheel_*`)** → *robustness*. Render a concise compatibility preamble at the top of every managed OpenCode skill body that maps logical `flywheel_*` names to the actual `flywheel_flywheel_*` prefix (or rewrites every call site — the robustness plan prefers the preamble to minimize diffs against source). Runtime smoke asserts a real `flywheel_flywheel_get_skill({ name: "agent-flywheel:start_planning" })` call returns the bundled body, not the disk fallback. Reason: this is the highest-severity correctness gap in the manual port today and nobody else saw it. Without it, every `flywheel_get_skill` reference silently misses at runtime.
...
10. Compatibility validator reports zero ERROR-class findings; any WARN is on the reviewed allowlist in `opencode/compatibility.json` with a reason.
...
14. Runtime smoke: `opencode` starts, `/flywheel-doctor` reaches green, `/start` prints the banner with the correct version (read from the injected repo path), one `flywheel_flywheel_get_skill({ name: "agent-flywheel:start_planning" })` returns the body from the bundle (not the disk fallback), one `question`-tool prompt renders correctly, one Agent-Mail guard block/allow pair works.
...
### T5. Semantic sweep as reviewable overlays
`depends_on: [T3]`
Translate the current 197 `AskUserQuestion` invocations to OpenCode's `question` tool contract (schema-aware, not identifier-rename). Replace the 77 `Agent(...)`, 66 `Team*`/`SendMessage`, and 52 agent-lifecycle `Task*` passages with NTM-first workflows or explicitly-degraded OpenCode behavior via patches in `opencode/patches/`. Remove executable `~/.claude/teams/` and `~/.claude/plugins/cache/` remediation blocks. Encode narrow retained forms with reasons in `opencode/compatibility.json`. Acceptance: staged compatibility scan has zero unapproved ERROR-class findings; every retained `agent-flywheel:` prefix is an MCP bundle identifier; every retained Claude provider name is in a provider/NTM context; a deliberately introduced new Claude tool call or a stale patch hunk fails before install.
...
2. **How aggressively to sweep semantic Claude-isms in v1.** Correctness's original position was mechanical-only: v1 emits a `--stale-report` counting the semantic-delta references, and a follow-up cycle does the translation. Ergonomics wanted post-render validation with grouped errors. Robustness wanted full sweep via checked-in patches, fail-closed. **Position taken:** adopt robustness's ambition — do the semantic sweep in v1 via patches — because the manual port today has 197 executable `AskUserQuestion` calls, 77 `Agent(...)` spawns, and 66 `Team*/SendMessage` calls that break at runtime; a copy that ships those unchanged is not "hardened." **Trade-off:** T5 significantly expands v1 scope. **What would flip it:** if the user prefers a v1 ship-date over completeness, drop T5 to a stale-report and defer patches to v2.
...
4. **MCP tool-prefix strategy — preamble vs. rewrite.** Robustness proposed a compatibility preamble at the top of each managed skill that maps logical `flywheel_*` names to `flywheel_flywheel_*`; the alternative is to rewrite every call site. **Position taken:** preamble, because it minimizes diff against source and keeps the port a thin translation layer. **What would flip it:** if the preamble mechanism turns out to not be respected by OpenCode's skill runtime, fall back to call-site rewrites (which is a straightforward transform with a fixture).
